...
# ==================== 2. 解析解函数 ====================
def analytical_displacement(x, y, params): # 计算任一点（x,y）的位移
    """Timoshenko梁理论解析解"""
...
def analytical_stress(x, y, params): # 计算任一点（x,y）的应力
    """计算解析解的应力分量"""
...
def generate_mesh(params, nx, ny):
    ...
    X, Y = np.meshgrid(x, y) # 生成网格点（将x和y组合成二维网格）得到网格的x坐标矩阵和y坐标矩阵
    nodes = np.column_stack((X.flatten(), Y.flatten()))  
    # 将网格点转换为节点列表（每行一个节点的[x,y]坐标）
    # nodes是一个二维数组，第一维是节点数，第二维是每个节点的坐标（x,y），第二维 0 是 x，1 是 y
    elements = []  # 单元列表（每个单元包含4个节点索引）
    for j in range(ny-1):
        for i in range(nx-1):
            # 四边形单元节点索引（逆时针顺序） 3在2上方，4在1上方，编号要大一行（nx）
            n1 = j*nx + i
            n2 = j*nx + i + 1
            n3 = (j + 1)*nx + i + 1
            n4 = (j + 1)*nx + i
    ...
    return nodes, np.array(elements), nx, ny  # 返回节点、单元和网格尺寸

# ==================== 4. 有限元计算 ====================
def fem_solution(nodes, elements, params):
    """执行有限元计算"""
    nnodes = nodes.shape[0]
    # nodes.shape的结果是（节点数，2），【0】表示取第一维度，这里为节点数
    # numpy 数组的 shape 属性返回一个元组，描述数组的维度，即说明数组的形状为多少行多少列：（行数，列数）
    ...
    # nodes[i,0]表示第 i 个节点的 x 坐标
    # np.isclose用于判断两个数是否近似相等，这里用于判断节点的 x 坐标是否接近 0
    fixed_dofs = []
    ...
    # np.setdiff1d(A, B)计算 A 中不在 B 中的元素，即得到自由自由度的编号列表
    K_free = K[free_dofs,:][:,free_dofs]
    # 从整体刚度矩阵K中提取只包含自由自由度的子矩阵
    F_eff = F[free_dofs] - K[free_dofs,:][:,fixed_dofs] @ U[fixed_dofs]# 计算有效力向量
    U[free_dofs] = spsolve(K_free, F_eff)# 求解自由自由度的位移
    
    return U#最终返回包含所有节点 x、y 方向位移的向量U（长度为2*nnodes），后续可基于此计算应力等结果。
    ...
    for e, elem in enumerate(elements):
        ...
        for i in range(params.ngp):
            for j in range(params.ngp):
                ...
                epsilon_exact = np.array([epsilon_xx, epsilon_yy, gamma_xy])
                

                # 解析解应力（通过本构矩阵计算）
                sigma_exact = D @ epsilon_exact

                # 计算误差的双点积
                d_sigma = sigma_exact - sigma_h
                d_epsilon = epsilon_exact - epsilon_h
                integral += np.dot(d_sigma, d_epsilon) * detJ * weight * params.t
...
def convergence_analysis(params, grid_sizes):
    ...
    for nx, ny in grid_sizes:
        ...
        print(f"网格尺寸: {nx}x{ny}, h={h:.4f}, 总L2误差={total_err:.6e}, 能量误差={energy_err:.6e}")

    # 计算总位移收敛率 (使用最后几个点进行线性拟合)
    ...
    return h_list, u_errors, v_errors, total_errors, energy_errors, total_convergence_rate, energy_convergence_rate
...
def plot_results(nodes, elements, U_fem, params, nx, ny):
    ...
    # ========== 第四个Figure：收敛率分析图（包含能量误差） ==========
    # 定义要测试的网格尺寸序列
    grid_sizes = [(10, 5), (20, 10), (40, 20), (80, 40), (160, 80)]
    h_list, u_errors, v_errors, total_errors, energy_errors, total_conv_rate, energy_conv_rate = convergence_analysis(params, grid_sizes)
    
    fig4 = plt.figure(figsize=(10, 8))
    fig4.suptitle('Convergence Analysis', fontsize=16)
    ...
    ax15.legend()
    ax15.set_title('Convergence of FEM solution with mesh refinement')
    ...
    fig4.savefig('convergence_analysis.png', dpi=300, bbox_inches='tight')
